...
%%%%% TODO algorithm for finding similar items from candidate pairs
...
%%% TODO: penso non sia necessario usare la CDF nel primo step. Altrimenti si vanno a penalizzare troppo features che non appaiono tante volte 
%%% ANSWER: Per me ci stava perchè l'esempio che avevo messo nel DOC capitava, però di per se ci sono altre penalizzazioni che agiscono quindi si può tralasciare!

% To conclude the feature rating process, it is important to penalize features that were used and graded a few times. To do so, each feature’s score is multiply by a weight given form the cumulative distribution function (CDF) of the number of observations of a feature in the query set
